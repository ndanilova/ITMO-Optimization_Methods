...
- `stages` — структура данных, содержащая для каждого этапа (1, 2, 3) вероятности трёх ситуаций (благоприятной, нейтральной, негативной) и соответствующие множители доходности для активов
- `initial` — начальное состояние инвестиционного портфеля: $\text{CB1} = 100$, $\text{CB2} = 800$, $\text{Dep} = 400$, $\text{Cash} = 600$
- `step = 25` — шаг управления, определяющий минимальное изменение объёма активов
- `commissions` — ставки комиссий: $\alpha_1 = 0.04$ (ЦБ1), $\alpha_2 = 0.07$ (ЦБ2), $\alpha_d = 0.05$ (депозиты)
- `minimal` — минимальные значения: $m_1 = 30$, $m_2 = 150$, $m_d = 100$

**Выходные результаты:**
- `optimal_value` — максимальное ожидаемое конечное богатство $\mathbb{E}[W_3]$
- `optimal_policy` — оптимальная последовательность управлений $\{u_k^*\}_{k=1}^2$

**Структура алгоритма**

Алгоритм реализует метод динамического программирования с **обратным проходом** (backward induction) и состоит из четырёх основных этапов:

**Этап 1: Дискретизация пространства состояний**

```python
# В коде это реализовано через перебор с шагом 25
step = 25
# Определение диапазонов для каждого актива
cb1_range = range(30, max_val + step, step)  # от минимального до максимума
```

Состояние системы на каждом этапе $k$ описывается вектором:
$$S_k = (x_1^k, x_2^k, d^k, c^k)$$
где:
- $x_1^k$ — объём средств в ЦБ1
- $x_2^k$ — объём средств в ЦБ2
- $d^k$ — объём средств в депозитах
- $c^k$ — свободные денежные средства

Из-за непрерывности состояний применяется **дискретизация** с шагом 25 д.е., что преобразует задачу в перебор конечного числа состояний.

**Этап 2: Терминальный расчёт (этап 3)**

Для последнего этапа ($k = 3$) функция Беллмана вычисляется непосредственно по формуле математического ожидания:

```python
def calculate_terminal_value(cb1, cb2, dep, cash):
    expected = 0
    for scenario in ['good', 'neutral', 'bad']:
        p = stages[3][scenario]['p']
        mult_cb1 = stages[3][scenario]['cb1']
        mult_cb2 = stages[3][scenario]['cb2']
        mult_dep = stages[3][scenario]['dep']
        
        value = cb1 * mult_cb1 + cb2 * mult_cb2 + dep * mult_dep + cash
        expected += p * value
    return expected
```

Математически это соответствует:
$$F_3(x_1, x_2, d, c) = \sum_{j \in J} p_j^3 \left[ x_1 \cdot r_{1j}^3 + x_2 \cdot r_{2j}^3 + d \cdot r_{dj}^3 + c \right]$$

где $J = \{\text{благ., нейтр., негат.}\}$ — множество возможных ситуаций.

**Этап 3: Обратный проход (этапы 2 и 1)**

Выполняется итерация от предпоследнего этапа к первому:
...
2. **Перебор допустимых управлений:**
   ...
   Для каждого $u$ проверяются ограничения:
   - $x_1 + \Delta_1 \geq m_1$, $x_2 + \Delta_2 \geq m_2$, $d + \Delta_d \geq m_d$
   - $c - (\Delta_1 + \Delta_2 + \Delta_d) - \text{comm}(u) \geq 0$
   
   где комиссия вычисляется как:
   $$\text{comm}(u) = \alpha_1 |\Delta_1| + \alpha_2 |\Delta_2| + \alpha_d |\Delta_d|$$

3. **Вычисление ожидаемого значения:**
   Если управление допустимо, вычисляется:
...
4. **Выбор оптимального управления:**
   Если text{expected} > \text{best_value}, то:
...
```python
for buy in range(0, initial['cash'], step):
    for sell in range(0, max_sell, step):
        # Проверка ограничений и вычисление expected
        if expected > best_value:
            best_value = expected
            best_action = (buy, sell, ...)
```

**Этап 4: Восстановление оптимальной траектории**
...
В алгоритме используется **критерий Байеса** — максимизация математического ожидания конечного капитала:
$$\max \mathbb{E}[W_3] = \max \sum_{j \in J} p_j \cdot W_3(j)$$

где $W_3(j)$ — конечный капитал при реализации сценария $j$.

**Особенности реализации в коде**

**Оптимизация перебора**
Вместо полного перебора всех возможных управлений используется эвристический подход:
- Рассматриваются только управления, изменяющие активы на $\{-50, -25, 0, 25, 50\}$
- Учитывается экономическая логика: покупка актива с максимальной ожидаемой доходностью

**Расчёт ожидаемых доходностей**
Для упрощения вычислений предварительно рассчитываются средние множители доходности:
```python
exp_cb1_1 = 0.6*1.2 + 0.3*1.05 + 0.1*0.8 = 1.115
exp_cb2_1 = 0.6*1.1 + 0.3*1.02 + 0.1*0.95 = 1.061
exp_dep_1 = 0.6*1.07 + 0.3*1.03 + 0.1*1.0 = 1.051
```

**Учёт комиссий**
Комиссии учитываются в ограничениях и при расчёте нового состояния:
```python
# Проверка возможности покупки с учётом комиссии
max_buy = int(cash / (1 + commission_rate))
```

**Проверка ограничений**
Перед применением управления выполняется комплексная проверка:
1. Минимальные значения активов
2. Неотрицательность свободных средств
3. Целочисленность изменений (кратность шагу)

**Результаты работы алгоритма**

Алгоритм определяет оптимальную стратегию:
- **Этап 1:** Купить ЦБ1 на 575 д.е.
- **Этап 2:** Не совершать сделок (или минимальную продажу)
- **Этап 3:** Не совершать сделок

С математическим ожиданием конечного капитала:
$$\mathbb{E}[W_3] = 2026\ \text{д.е.}\quad (\text{доходность } 6.6\%)$$
